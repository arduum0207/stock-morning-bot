...
> ⚠️ **루틴은 반드시 "대상 레포"를 고를 수 있는 화면(`/schedule`, 웹/데스크톱 앱의 Routines)에서 만드세요.**
> 채팅으로 "루틴 만들어줘" 라고 시켜 **에이전트가 대신 만든 루틴에는 레포가 붙지 않습니다.** 그러면 매일 아침
> 빈 작업공간에서 세션이 뜨고 `ROUTINE.md` 를 못 찾아 **아무 알림도 없이 실패**합니다.
> 루틴 목록에서 각 루틴이 어떤 레포를 보는지 확인하고, 레포가 없는 루틴은 지우세요.
> (이미 그런 루틴을 쓰고 있다면 → [트러블슈팅](#-트러블슈팅)의 `ROUTINE.md 를 못 찾음` 항목)
...
| `ROUTINE.md 를 못 찾음` / 아침 실행이 첫 단계에서 죽음 | **루틴에 레포가 안 붙은 것.** 레포 없이 만들어진 루틴(에이전트가 MCP로 생성한 루틴)은 빈 작업공간에서 뜹니다. 해결: ① 앱의 Routines 에서 **대상 레포를 지정해 루틴을 다시 만들고** 레포 없는 루틴은 삭제, 또는 ② 그 루틴의 프롬프트 앞에 [부트스트랩 블록](ROUTINE.md#0-1-부트스트랩-레포가-안-붙은-루틴용)을 붙여 세션이 스스로 clone 하게 하기 |
