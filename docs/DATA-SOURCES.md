...
전부 **실제로 호출해서 확인한 결과**다(실측 2026-08 · 갱신 2026-09). 소스를 추가·교체할 때 여기부터 보면 된다.
...
| `nasdaq` | api.nasdaq.com | 불필요(브라우저 UA) | US 실적 일정 + **발표된 결과** + 대형주 실적 |
...
## 함정 — Nasdaq 실적 캘린더는 날짜에 따라 다른 필드를 준다

같은 `/api/calendar/earnings?date=` 인데 **예정일과 지난 날짜의 응답 스키마가 다르다.**

| 날짜 | 오는 필드 | 안 오는 필드 |
|---|---|---|
| 예정 | `epsForecast`, `lastYearEPS`, `lastYearRptDt` | `eps`, `surprise` (키 자체가 없음) |
| 지난 | `eps`(실제), `surprise`(%) | `lastYearEPS` |

→ 미래만 조회하면 발표가 끝나는 순간 그 종목이 목록에서 사라진다. `nasdaq.ts` 가
직전 2영업일을 함께 훑는 이유다. 전년비(YoY)는 **예정 건에서만** 공짜로 얻을 수 있다.

KST 아침에 도는 루틴은 UTC 로 아직 전날이라, "오늘" 로 요청한 날이 곧 방금 장이 끝난 날이다.
그날 장마감 후 발표분은 이미 `eps` 가 차서 온다 → 발표 여부는 날짜가 아니라 `eps` 유무로 판정한다.

## 함정 — 적자 기업의 서프라이즈 %

컨센이 음수면 `(실제-컨센)/컨센` 이 부호까지 뒤집혀 **정반대로 읽힌다.**

```
NBIS   컨센 -$0.67 → 실제 -$0.12   (적자폭을 크게 줄인 호실적)
공식대로 → -82%                     ("82% 미스" 로 읽힘)
BLSM   컨센 -$0.84 → 실제 -$8.75   → -941%  (실제 의미는 "적자폭 10배")
```

→ `common.ts` 의 `surprisePct()` 가 **컨센 0 이하면 null** 을 준다. 그땐 EPS 절대값과
방향(덜 마이너스면 상회)으로 말해야 한다. watchlist 의 NBIS·MVIS·INVZ·AEVA 가 여기 해당.
